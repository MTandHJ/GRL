...
# save the checkpoint
def save_checkpoint(path, gf, gc, gd, optimizer, lr_scheduler, epoch):
...
# load the checkpoint
def load_checkpoint(path, gf, gc, gd, optimizer, lr_scheduler):
    path = path + "/model-optim-lr_sch-epoch.tar"
    checkpoint = torch.load(path)
    gf.load_state_dict(checkpoint['gf'])
    gc.load_state_dict(checkpoint['gc'])
